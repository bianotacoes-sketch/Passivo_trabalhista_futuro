...
DATA_REFERENCIA AS (
    SELECT 
        ...
        FUN.CODFUNC,
        CASE 
            WHEN AVI.CODFUNC IS NOT NULL AND TO_DATE(&DATA_PARAMETRO, 'DD/MM/YYYY') > AVI.DTFIMAVISO THEN AVI.DTFIMAVISO
            ELSE TO_DATE(&DATA_PARAMETRO, 'DD/MM/YYYY')
        END AS DATA_REF
    FROM TFPFUN FUN
...
COLABORADORES AS (
    SELECT
        ...
        CAR.DESCRCARGO,
        CASE
            WHEN DAY(ISNULL(FUN.DTDEM, DR.DATA_REF)) = DAY(EOMONTH(ISNULL(FUN.DTDEM, DR.DATA_REF))) THEN 30
            ELSE DAY(ISNULL(FUN.DTDEM, DR.DATA_REF)) 
        END AS DIAS_SALDO_SALARIO,
...
FERIAS_PROPORCIONAIS AS (
    SELECT 
        ...
        ROUND(((DATEDIFF(MONTH, MAX(DTINIAQUI), ISNULL(COL.DTDEM, DR.DATA_REF)))), 0) % 12) AS FERPROP
    FROM COLABORADORES COL
    INNER JOIN DATA_REFERENCIA DR ON DR.CODEMP = COL.CODEMP AND DR.CODFUNC = COL.CODFUNC
    LEFT JOIN FERIAS FER ON FER.CODEMP = COL.CODEMP AND FER.CODFUNC = COL.CODFUNC
    GROUP BY COL.CODEMP, COL.CODFUNC, SALBASE, COL.DTDEM, DR.DATA_REF
),
...
SELECT DISTINCT
    ...
    (COL.SALBASE / 30 * (30 + (FLOOR(DATEDIFF(DAY, COL.DTADM, ISNULL(COL.DTDEM, DR.DATA_REF)) / 365) * 3)) / 12 * ROUND((30 + (FLOOR(DATEDIFF(DAY, COL.DTADM, ISNULL(COL.DTDEM, DR.DATA_REF)) / 365) * 3)) / 30, 0) + ISNULL(MD2.VALOR, 0)) / 3 AS TERCO_FERIAS_PROP_API,
    ...
    (ROUND((ISNULL(COL.DECTERSALARIO, 0) + ISNULL(MD2.VALOR_2, 0)) * 0.08, 2)) + ((COL.SALBASE / 30 * COL.DIAS_SALDO_SALARIO) * 0.08) + (COL.SALDO_ANTERIOR_FGTS + SFG.SALDO_SANKHYA_FGTS) * 0.4 AS FGTS_MULTA,
...
select 
    ...
    DIAS_SALDO_SALARIO,
    SALDO_SALARIO,
    case when avi.codfunc is not null then 0 else AVISO_PREVIO_INDENIZADO end AVISO_PREVIO_INDENIZADO,
    case when avi.codfunc is not null then 0 else MEDIA_AVISO_PREVIO_INDENIZADO end MEDIA_AVISO_PREVIO_INDENIZADO,
    case when avi.codfunc is not null then 0 else FGTS_AVISO_IND end FGTS_AVISO_IND,
    case when avi.codfunc is not null then 0 else DECTER_API end DECTER_API,
    case when avi.codfunc is not null then 0 else MEDIA_DECTER_API end MEDIA_DECTER_API,
    case when avi.codfunc is not null then 0 else FGTS_DECTERC_API end FGTS_DECTERC_API,
    case when avi.codfunc is not null then 0 else FERIAS_PROP_API end FERIAS_PROP_API,
    case when avi.codfunc is not null then 0 else TERCO_FERIAS_PROP_API end TERCO_FERIAS_PROP_API,
    case when avi.codfunc is not null then 0 else MEDIA_FERIAS_API end MEDIA_FERIAS_API,
...
SELECT 
    ...
    DIAS_SALDO_SALARIO,
    SALDO_SALARIO,
    CASE WHEN avi.codfunc IS NOT NULL THEN 0 ELSE AVISO_PREVIO_INDENIZADO END AS AVISO_PREVIO_INDENIZADO,
    CASE WHEN avi.codfunc IS NOT NULL THEN 0 ELSE MEDIA_AVISO_PREVIO_INDENIZADO END AS MEDIA_AVISO_PREVIO_INDENIZADO,
    CASE WHEN avi.codfunc IS NOT NULL THEN 0 ELSE FGTS_AVISO_IND END AS FGTS_AVISO_IND,
    CASE WHEN avi.codfunc IS NOT NULL THEN 0 ELSE DECTER_API END AS DECTER_API,
    CASE WHEN avi.codfunc IS NOT NULL THEN 0 ELSE MEDIA_DECTER_API END AS MEDIA_DECTER_API,
    CASE WHEN avi.codfunc IS NOT NULL THEN 0 ELSE FGTS_DECTERC_API END AS FGTS_DECTERC_API,
    CASE WHEN avi.codfunc IS NOT NULL THEN 0 ELSE FERIAS_PROP_API END AS FERIAS_PROP_API,
    CASE WHEN avi.codfunc IS NOT NULL THEN 0 ELSE TERCO_FERIAS_PROP_API END AS TERCO_FERIAS_PROP_API,
    CASE WHEN avi.codfunc IS NOT NULL THEN 0 ELSE MEDIA_FERIAS_API END AS MEDIA_FERIAS_API,
    FERIAS_PROPORCIONAIS,
    TERCO_FERIAS_PROPORCIONAIS,
    MEDIA_FERIAS_PROPORCIONAIS,
    FERPROP,
