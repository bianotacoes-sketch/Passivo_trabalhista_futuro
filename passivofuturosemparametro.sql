...
COLABORADORES AS (
    SELECT
		...
		30 + (datediff(YEAR,FUN.DTADM,P.DATA_PARAM)*3) AS DIAS_AVISO_INDENIZADO,
...
select 
   ...
   CASE 
    ...
    WHEN TIPO_AVISO_PREVIO <> 'Trabalhado' AND YEAR(DATEADD(DAY, -DIAS_AVISO_INDENIZADO, GETDATE())) = YEAR(GETDATE())
        THEN DIAS_AVISO_INDENIZADO * (DECTERSALARIO / 12)
    ELSE (DIAS_AVISO_INDENIZADO - DATEDIFF(DAY, DATEFROMPARTS(YEAR(GETDATE()), 1, 1), GETDATE())) * (DECTERSALARIO / 12)
END AS DECTER_API,
...
CASE WHEN TIPO_AVISO_PREVIO ='Trabalhado' then 
(ISNULL(SALDO_FGTS,0) + ISNULL(SALDO_SALARIO* 0.08,0) + ISNULL(FGTS,0))*0.4
when TIPO_AVISO_PREVIO ='Indenizado' then 
(ISNULL(SALDO_FGTS,0) + ISNULL(SALDO_SALARIO* 0.08,0) + ISNULL(FGTS,0))*0.4
+ isnull(FGTS_AVISO_IND,0) + isnull(FGTS_DECTERC_API,0) 
end AS MULTA_FGTS,--(ISNULL(SALDO_FGTS, 0) + ((ISNULL(SALBASE, 0)/30) * ISNULL(DIAS_SALDO_SALARIO, 0)) * 0.08 + ISNULL(DECTERSALARIO, 0) * 0.08 + ISNULL(FGTS_DECTERC_API, 0) + ISNULL(FGTS_AVISO_IND, 0)) * 0.4 


-- TOTAL FGTS MEDIDA
CASE WHEN p.TIPO_AVISO_PREVIO ='Trabalhado' THEN 
ISNULL(SALDO_FGTS,0) -- saldo_fgts
+
isnull(FGTS,0) -- fgts13
+
((ISNULL(SALBASE, 0)/30) * ISNULL(DIAS_SALDO_SALARIO, 0)) * 0.08 --fgtsmes
+
(ISNULL(SALDO_FGTS,0) + ISNULL(SALDO_SALARIO* 0.08,0) + ISNULL(FGTS,0))*0.4 -- multa_fgts
when p.TIPO_AVISO_PREVIO ='Indenizado' then
ISNULL(SALDO_FGTS,0) -- saldo_fgts
+
isnull(FGTS,0) -- fgts13
+
...
+
(ISNULL(SALDO_FGTS,0) + ISNULL(SALDO_SALARIO* 0.08,0) + ISNULL(FGTS,0))*0.4
+ isnull(FGTS_AVISO_IND,0) + isnull(FGTS_DECTERC_API,0) --multa_fgts
 END  TOTAL_FGTS
...
ISNULL(MEDIA_DECTERCSALARIO,0) AS
TOTAL_VERBAS_RESCISORIAS 
,
ISNULL(SALDO_SALARIO,0) +
ISNULL(AVISO_PREVIO_INDENIZADO,0) +
ISNULL(MEDIA_AVISO_PREVIO_INDENIZADO,0) +
...
ISNULL(TERCO_FERIAS_VENCIDAS,0) +
ISNULL(DECTERSALARIO,0) +
ISNULL(MEDIA_DECTERCSALARIO,0) + ISNULL(TOTAL_FGTS,0) AS TOTAL_PASSIVO_TRABALHISTA
from base_dois
