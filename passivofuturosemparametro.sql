...
SELECT DISTINCT
    ...
    (COL.SALBASE --+ ISNULL(CPS.COMPSALDO, 0)
	
	) / 30 * DIAS_SALDO_SALARIO AS SALDO_SALARIO,
   ...
   --TÔ FAZENDO ESSE ISNULL(COL.DECTER_API,0) AS FGTS_DECTERC_API,
...
END AS DECTER_API,
 ...
 CASE 
    WHEN TIPO_AVISO_PREVIO = 'Trabalhado' THEN 0 
    WHEN TIPO_AVISO_PREVIO <> 'Trabalhado' AND DIAS_AVISO_INDENIZADO < 15 THEN 0
    WHEN DIAS_AVISO_INDENIZADO >= 15 AND DIAS_AVISO_INDENIZADO < 45 THEN (1 * SALBASE/12)*0.08
    WHEN DIAS_AVISO_INDENIZADO >= 45 THEN 
        CASE 
            WHEN (DIAS_AVISO_INDENIZADO/30.0 - FLOOR(DIAS_AVISO_INDENIZADO/30.0)) >= 0.5 
            THEN CEILING(DIAS_AVISO_INDENIZADO/30.0) * (SALBASE/12)
            ELSE (FLOOR(DIAS_AVISO_INDENIZADO/30.0) * (SALBASE/12))*0.08
        END
