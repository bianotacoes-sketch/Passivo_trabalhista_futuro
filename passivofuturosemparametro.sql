...
PARAMETRO AS (
    SELECT 
        CONVERT(DATETIME, '31/01/2026', 103) AS DATA_PARAM,
...
FERIAS_PROPORCIONAIS AS (
    SELECT 
        ...
        col.SALBASE,
		case 
		WHEN COL.DTADM = INI_PERIODO_AQUISITIVO AND DR.DATA_REF >= DATEADD(YEAR,1,COL.DTADM) 
		...
		---adição feita de condiçãao de a data parãmetro ser maior ou igual a 15 
    ...
    GROUP BY COL.CODEMP, COL.CODFUNC, SALBASE, COL.DTDEM, DR.DATA_REF,COL.DTADM,COL.DATA_PARAM, INI_PERIODO_AQUISITIVO
),
...
when 
TIPO_AVISO_PREVIO ='Indenizado' then
...
CASE WHEN TIPO_AVISO_PREVIO ='Indenizado' then 
((ISNULL(SALDO_FGTS,0) + ((ISNULL(SALDO_SALARIO,0) + ISNULL(INSALUBRIDADE,0) + ISNULL(PERICULOSIDADE,0))*0.08)+
ISNULL(FGTS_DECTERCEIRO,0)+ISNULL(FGTS_DECTERC_API,0) +ISNULL(FGTS_AVISO_IND,0)))*0.4
...
--select * from tfpfun where codfunc = 22490 
